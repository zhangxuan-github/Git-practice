function [thoughputArray, maxThroughputArray, transmitSizeArray] = nrDownlinkTransmit(gNBs, UEs, globalConfig)
    
    % Downlink transmitter for NR
    % 删除现有并行池（如果存在）
    openParallelPool();

    [thoughputArray, maxThroughputArray, transmitSizeArray] = performParallelOperation(gNBs, UEs, globalConfig);

    closeParallelPool();
end

function [resThoughputArray, resMaxThroughputArray, resTransmitSizeArray] = performParallelOperation(gNBs, UEs, globalConfig)
    % 预分配结果数组
    nUEs = length(UEs);
    resThoughputArray = zeros(1, nUEs);
    resMaxThroughputArray = zeros(1, nUEs);
    resTransmitSizeArray = zeros(1, nUEs);
    
    tic_parfor = tic;
    parfor i = 1:nUEs
        % 执行传输并获取结果
        [resThoughput, resMaxThroughput, resTransmitSize] = downlinkTransmit(gNBs{i}, UEs{i}, globalConfig);
...
function [resThoughput, resMaxThroughput, resTransmitSize] = downlinkTransmit(gNB, UE, globalConfig)
    ...
    simPara.PerfectChannelEstimator = false;
    simPara.DisplaySimulationInformation = globalConfig.DisplaySimulationInformation;
    simPara.DisplayDiagnostics = false;

    simPara.Carrier = nrCarrierConfig;         % Carrier resource grid configuration
    simPara.Carrier.NSizeGrid = UE.allocatedRbs;            % Bandwidth in number of resource blocks (51 RBs at 30 kHz SCS for 20 MHz BW)
    simPara.Carrier.SubcarrierSpacing = gNB.subcarrierSpacing;    % 15, 30, 60, 120 (kHz)
    simPara.Carrier.CyclicPrefix = 'Normal';   % 'Normal' or 'Extended' (Extended CP is relevant for 60 kHz SCS only)，写死
    ...
    if simPara.PDSCH.NumCodewords > 1                             % Multicodeword transmission (when number of layers being > 4)
        ...
        simPara.PDSCHExtension.TargetCodeRate = [UE.TargetCodeRate UE.TargetCodeRate]/1024;   % Code rate used to calculate transport block sizes
    else
        simPara.PDSCH.Modulation = UE.Modulation;                       % 'QPSK', '16QAM', '64QAM', '256QAM'
        simPara.PDSCHExtension.TargetCodeRate = UE.TargetCodeRate/1024;         % Code rate used to calculate transport block sizes
    end
    ...
    simPara.NRxAnts = UE.numAntennas;                                            % Number of PDSCH receive antennas (1,2,4,8,16,32,64,128,256,512,1024)
    ...
    disp(estChannelGridAnts);
    disp(pdschextra.PRGBundleSize);
    ...
    fprintf('Throughput(%%) for %d frame(s) = %.4f\n',simLocal.NFrames,simThroughput*100/maxThroughpu);
