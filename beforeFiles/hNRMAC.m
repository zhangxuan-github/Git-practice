    ...
    methods (Access = public)
        ...
        function addLogicalChannelInfo(obj, logicalChannelConfig, varargin)
            ...
            if ~isempty(obj.LogicalChannelsConfig{ueIdx, logicalChannelConfig.LCID})
                error('nr5g:hNRMAC:DuplicateLogicalChannel','Duplicate logical channel configuration is present for LCID ( %d ) in UE ( %d ).', logicalChannelConfig.LCID, ueIdx);
            end
            ...
            if isfield(logicalChannelConfig, 'PBR')
                ...
                if ~ismember(logicalChannelConfig.PBR, obj.PBR)
                    error('nr5g:hNRMAC:InvalidPBR','PBR ( %d ) must be one of the set (0, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, Inf).', logicalChannelConfig.PBR);
                end
            end
            ...
            if isfield(logicalChannelConfig, 'BSD')
                ...
                if ~ismember(logicalChannelConfig.BSD, obj.BSD)
                    error('nr5g:hNRMAC:InvalidBSD','BSD ( %d ) must be one of the set (5, 10, 20, 50, 100, 150, 300, 500, 1000).', logicalChannelConfig.BSD);
                end
            end
            ...
            % gNB
            if ~obj.MACType
                if obj.LCGPriority(logicalChannelConfig.RNTI,logicalChannelConfig.LCGID + 1) > logicalChannelConfig.Priority
                    obj.LCGPriority(logicalChannelConfig.RNTI,logicalChannelConfig.LCGID + 1) = logicalChannelConfig.Priority;
                end
            end
        end
        ...
        function pdu = constructMACPDU(obj, tbs, varargin)
            ...
            if linkDir
                ...
                pdu = [vertcat(dataSubPDUList{:}); vertcat(controlPDUList{:}); paddingSubPDU];
            else
...
methods (Access = private)
        ...
        function [dataSubPDUList, utilizedGrant] = getMACSubPDUs(obj, ueIdx, lchIdx, grantSize, remainingTBS)
            ...
            for sduIdx = 1:numel(macSDUs)
                dataSubPDUList{dataPDUIdx} = nrMACSubPDU(linkDir, lchIdx, macSDUs{sduIdx});
