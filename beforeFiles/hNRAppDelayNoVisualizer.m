 classdef hNRAppDelayNoVisualizer < handle
...
%         %EnablePlots Switch to turn on/off the delay metrics plots
%         % It is a logical scalar. Set the value as true to enable the
%         % plots. By default the plots ar disabled
%         EnablePlots = false;
    end
   ...
   properties(Access = private, Hidden)
    ...
    methods (Access = public)
        function obj = hNRAppDelayNoVisualizer(param, varargin)
            ...
            validateattributes(param.NumFramesSim, {'numeric'}, {'nonempty', 'integer', 'scalar', 'finite', '>', 0}, 'simParameters.NumFramesSim', 'NumFramesSim');
            ...
            validateattributes(obj.NumMetricsSteps, {'numeric'}, {'nonempty', 'integer', 'scalar', '>', 0, '<=', param.NumFramesSim * (param.SCS / 15) * 10}, 'simParameters.NumMetricsSteps', 'NumMetricsSteps');
            ...
            obj.DLDelayInfo = zeros(totalNumUEs, 2);
            obj.ULDelayInfo = zeros(totalNumUEs, 2);
...
%                 else
%                     error('nr5g:hNRMetricsVisualizer:InvalidLCHInfo', 'LCHInfo must be nonempty.');
%                 end
%             end
% 
...
%         end
% 
%         function addDelayVisualization(obj, varargin)
%             if nargin == 2
%                 obj.PhyLogger = varargin{1};
%             end
% 
%             % Create and configure the timescope
%             if isempty(obj.DelayVisualization)
%                 obj.DelayVisualization = timescope('Name', '5G NR Time-Delay Visualization');
%             end
% 
%             % Maximum number of node delay metrics allowed to plot
%             numUEs = size(obj.UEOfInterestListInfo, 1);
%             if numUEs > obj.MaxPhyMetricNodes
%                 numUEs = obj.MaxPhyMetricNodes;
%             end
%             delayData = zeros(1, numUEs);
% 
%             set(obj.DelayVisualization, 'LayoutDimensions', [numel(obj.PlotIDs) 1], 'ShowLegend', true, ...
%                 'SampleRate', obj.NumMetricsSteps/obj.SimTime,'TimeSpanSource', 'property','ChannelNames', repmat(obj.UELegend(1:numUEs), [1 numel(obj.PlotIDs)]), 'TimeSpan', obj.SimTime);
% 
%             titles = {'5G-NR Downlink Delay', '5G-NR Uplink Delay'};
%             % Initialize the plots
%             if numel(obj.PlotIDs) == 1
%                 obj.DelayVisualization(delayData);
%             else
%                 obj.DelayVisualization(delayData, delayData);
%             end
% 
%             % Add the titles and legends
%             for idx=1:numel(obj.PlotIDs)
%                 obj.DelayVisualization.ActiveDisplay = idx;
%                 obj.DelayVisualization.YLimits = [0 1];
%                 obj.DelayVisualization.YLabel = ['Cell-' num2str(obj.CellOfInterest) ' Delay(ms)'];
%                 obj.DelayVisualization.Title = titles{obj.PlotIDs(idx)};
%             end
%         end
        ...
        function displayPerformanceIndicators(obj)
            ...
            if obj.EnableSchedulerMetricsPlots
                if ismember(obj.UplinkIdx, obj.PlotIDs) % Uplink stats
                    ...
                    fprintf('Peak UL Throughput: %0.2f Mbps. Achieved Cell UL Throughput: %0.2f Mbps\n', obj.PeakDataRate(obj.UplinkIdx), sum(ulThroughputDataRate));
                    disp(['Achieved UL Throughput for each UE: [' num2str(round(ulThroughputDataRate, 2)') ']']);
                    fprintf('Achieved Cell UL Goodput: %0.2f Mbps\n', sum(ulGoodPutDataRate)');
                    disp(['Achieved UL Goodput for each UE: [' num2str(round(ulGoodPutDataRate, 2)') ']']);
                ...
                if ismember(obj.DownlinkIdx, obj.PlotIDs) && obj.EnableSchedulerMetricsPlots % Downlink stats
                    ...
                    fprintf('Peak DL Throughput: %0.2f Mbps. Achieved Cell DL Throughput: %0.2f Mbps\n', obj.PeakDataRate(obj.DownlinkIdx), sum(dlThroughputDataRate));
                    disp(['Achieved DL Throughput for each UE: [' num2str(round(dlThroughputDataRate, 2)') ']']);
                    fprintf('Achieved Cell DL Goodput: %0.2f Mbps\n', sum(dlGoodputDataRate));
                    disp(['Achieved DL Goodput for each UE: [' num2str(round(dlGoodputDataRate, 2)') ']']);
            ...
            if obj.EnablePhyMetricsPlots
                if ismember(obj.UplinkIdx, obj.PlotIDs) % Uplink stats
                    ...
                    disp(['Block error rate for each UE in the uplink direction: [' num2str(round(ulBLER, 3)') ']']);
                    disp(['Number of error blocks for each UE in the uplink direction: [' num2str(obj.ULBLERInfo(:, 1)') ']']);
                    disp(['Number of total blocks for each UE in the uplink direction: [' num2str(obj.ULBLERInfo(:, 2)') ']']);
                    %disp(['Block error rate for each UE in the uplink direction: [' num2str(round(ulBLER, 3)') '=' num2str(obj.ULBLERInfo(:, 1)) '/' num2str(obj.ULBLERInfo(:, 2)) ']']);
                end
                if ismember(obj.DownlinkIdx, obj.PlotIDs) % Downlink stats
                    dlBLER = obj.DLBLERInfo(:, 1) ./ obj.DLBLERInfo(:, 2);
                    %disp(['Block error rate for each UE in the downlink direction: [' num2str(round(dlBLER, 3)') ']']);
                    disp(['Block error rate for each UE in the downlink direction: [' num2str(round(dlBLER, 3)') ']']);
                    disp(['Number of error blocks for each UE in the downlink direction: [' num2str(obj.DLBLERInfo(:, 1)') ']']);
                    disp(['Number of total blocks for each UE in the downlink direction: [' num2str(obj.DLBLERInfo(:, 2)') ']']);
        ...
        function [dlPeakDataRate, ulPeakDataRate] = calculatePeakDataRate(~, simParameters)
            ...
            elseif ~ismember(simParameters.GNBTxAnts, [1,2,4,8,16,32,64,128,256,512,1024])
                error('nr5g:hNRMetricsVisualizer:InvalidAntennaSize',...
...
%             updateRLCMetrics(obj, metricInfo');
        ...
        function plotLivePhyMetrics(obj)
            ...
            if obj.VisualizationFlag ~= 1 % Downlink
                ...
                % added by cyl
                dlBLERInfo_1 = (dlBLERInfo(:, 1) - obj.DLBLERInfo(:, 1));
                dlBLERInfo_2 = (dlBLERInfo(:, 2) - obj.DLBLERInfo(:, 2)); 
                date_str=datestr(now,31);
                new_str = replace(date_str,' ','_'); 
                new_str=replace(new_str,':','');
                filename=sprintf('./log_data/dlblerinfo/bler_part_%s.mat',new_str);
                save(filename,"dlBLERInfo_1","dlBLERInfo_2");
                % added code end
            ...
            if obj.VisualizationFlag ~= 0 % Uplink
                ...
                blerData(obj.UplinkIdx, :) = ((ulBLERInfo(:, 1) - obj.ULBLERInfo(:, 1)) ./ (ulBLERInfo(:, 2) - obj.ULBLERInfo(:, 2)))';
                 % added by cyl
                ulBLERInfo_1 = (ulBLERInfo(:, 1) - obj.ULBLERInfo(:, 1));
                ulBLERInfo_2 = (ulBLERInfo(:, 2) - obj.ULBLERInfo(:, 2)); 
                date_str=datestr(now,31);
                new_str = replace(date_str,' ','_'); 
                new_str=replace(new_str,':','');
                filename=sprintf('./log_data/ulblerinfo/bler_part_%s.mat',new_str);
                save(filename,"ulBLERInfo_1","ulBLERInfo_2");
                % added code end
        ...
        function plotLiveDelayMetrics(obj)
            %plotLivePhyMetrics Plots the Phy live metrics

            ueList = obj.Nodes.UEs;
            numUEs = numel(ueList);

            delayData = zeros(2, numUEs); 
            dldelayInfo = zeros(numUEs,2);
            count = 1;  


            if obj.VisualizationFlag ~= 1 % Downlink
                for idx = 1:numUEs
                    dldelayInfo(count, :) = ueList{idx}.PhyEntity.Time_DLDelay;
                    count = count + 1;
                end
                delayData(obj.DownlinkIdx, :) = ((dldelayInfo(:, 1) - obj.DLDelayInfo(:, 1)) ./ (dldelayInfo(:, 2) - obj.DLDelayInfo(:, 2)))'; %两次指标输出时刻期间的 总和时延/计数=平均时延
                obj.DLDelayInfo = dldelayInfo;
            end

            if obj.VisualizationFlag ~= 0 % Uplink
                uldelayInfo = obj.Nodes.GNB.PhyEntity.Time_ULDelay;
                delayData(obj.UplinkIdx, :) = ((uldelayInfo(:, 1) - obj.ULDelayInfo(:, 1)) ./ (uldelayInfo(:, 2) - obj.ULDelayInfo(:, 2)))';
                obj.ULDelayInfo = uldelayInfo;
            end
            delayData = delayData(:, obj.UEOfInterestListInfo') ./ 1000  ;  %变量转置 换单位ms级别
            updateDelayMetrics(obj, delayData);
        end
        ...
        function updateMACMetrics(obj, throughput, resourceshare, goodput, bufferstatus)
            ...
            bufferstatus(isnan(bufferstatus)) = 0;
             ...
             % add the date to .mat file  --- by cyl 
            date_str=datestr(now,31);
            new_str = replace(date_str,' ','_'); 
            new_str=replace(new_str,':','');

            filename=sprintf('./log_data/throughput/throughput_part_%s.mat',new_str);
            save(filename,"throughput");
            filename=sprintf('./log_data/resourceshare/resourceshare_part_%s.mat',new_str);
            save(filename,"resourceshare");
            filename=sprintf('./log_data/goodput/goodput_part_%s.mat',new_str);
            save(filename,"goodput");
            filename=sprintf('./log_data/bufferstatus/bufferstatus_part_%s.mat',new_str);
            save(filename,"bufferstatus");
            % end add 
        ...
        function updatePhyMetrics(obj, blerData)
            ...
            new_str = replace(date_str,' ','_'); 
            new_str=replace(new_str,':','');
            filename=sprintf('./log_data/bler/bler_part_%s.mat',new_str);
            save(filename,"blerData");
            % end add 

            % Determine the maximum UEs to plot the metrics
            numUEs = size(blerData, 2); % Number of UEs获得列数
            if numUEs > obj.MaxPhyMetricNodes
                numUEs = obj.MaxPhyMetricNodes;
            end
        ...
        function updateDelayMetrics(obj, delayData)
            %updatePhyMetrics Update the Phy metrics

            delayData(isnan(delayData)) = 0; % To handle NaN

            % Determine the maximum UEs to plot the metrics
            numUEs = size(delayData, 2); % Number of UEs获得列数
            if numUEs > obj.MaxPhyMetricNodes
                numUEs = obj.MaxPhyMetricNodes;
            end
            ...
            new_str = replace(date_str,' ','_'); 
            new_str=replace(new_str,':','');
            filename=sprintf('./log_data/delay/delay_part_%s.mat',new_str);
            save(filename,"delayData");
            % end add 

            % Update the plots
            %if numel(obj.PlotIDs) == 1
                %obj.DelayVisualization(delayData(obj.PlotIDs, 1:numUEs));
            %else
                %obj.DelayVisualization(delayData(obj.DownlinkIdx, 1:numUEs), delayData(obj.UplinkIdx, 1:numUEs));
            %end
        end
    end
end