classdef (Abstract) hNRScheduler < handle
    ...
    properties (SetAccess = protected, GetAccess = public)
        ...
        %NumDLULPatternSlots Number of slots in DL-UL pattern (for TDD mode)DL-UL模式下的槽位数
        % The default value is 5 slots   5ms单周期？
        ...
        % many symbols before the transmission time.调度器确保 PUSCH 授权至少在传输时间之前到达 UE 的这些符号
        PUSCHPrepSymDur
        ...
        % The default value is 4 symbols.以符号数表示的最小时域分配（用于基于符号的调度）。
    ...
    properties (Access = protected)
        ...
        %SlotsSinceSchedulerRunDL 自上次 DL 调度程序运行以来经过的槽数（对于 FDD 模式）
         % 每个时隙递增，当它到达'SchedulerPeriodicity'，它被重置为零并且DL调度程序运行
    ...
    methods
        function obj = hNRScheduler(param)
            ...
            % SchedulerPeriodicity   - Scheduler run periodicity in slots (for FDD mode)-1
            ...
            % NumULSlots       - Number of full UL slots at the end of DL-UL pattern (for TDD mode)
            % PUSCHPrepTime    - PUSCH preparation time required by UEs (200 microseconds)
            ...
            % SSBIndex         - Index of the SSB associated with the UEs.It is a vector of length N, where 'N' 
            %                    is the number of UEs connected to the gNB. The element at position 'i' corresponds
            %                    to the SSB beam associated with the UE with RNTI 'i'
            ...
            obj.BufferStatusDLPerUE = zeros(numUEs, 1);
            obj.BufferStatusULPerUE = zeros(numUEs, 1);
            ...
            if isfield(param, 'RVSequence')
                harqProcess.RVSequence = param.RVSequence;
            else
                harqProcess.RVSequence = [0 3 2 1];
            end
            ...
            obj.CSIMeasurementDL = repmat(struct('RankIndicator', [], 'PMISet', [], 'CQI', [], 'CSIResourceIndicator', [], 'L1RSRP', []), numUEs, 1);
        ...
        function handleDLRxResult(obj, rxResultInfo)
            ...
            if rxResultInfo.RxResult % Rx success
                ...
                if harqProcess.RVIdx(1) == length(harqProcess.RVSequence)
                    ...
                    % further retransmissions for this packet 所有重传都失败
                    harqProcess.blkerr(1) = 0;
                    obj.HarqStatusDL{rnti, harqID+1} = []; % Mark the HARQ process as free
                    obj.HarqProcessesDL(rnti, harqID+1) = harqProcess;
                    obj.RetransmissionContextDL{rnti, harqID+1}= [];
    ...
    methods (Access = protected)
        ...
        function schedulerInput = createSchedulerInput(obj, linkDir, slotNum, eligibleUEs, selectedRank, rbgIndex, startSym, numSym)
            ...
            schedulerInput.bufferStatus = ueBufferStatus(eligibleUEs);
        ...
        function [rank, W] = selectRankAndPrecodingMatrixDL(obj, rnti, csiReport, numCSIRSPorts)
            ...
            if numCSIRSPorts == 1 || isempty(csiReport.PMISet)
        ...
        function populateDuplexModeProperties(obj, param)
            % Populate duplex mode dependent properties填充与双工模式相关的属性
        ...
        function populateTDDConfiguration(obj, param)
            ...
            % Validate the DL-UL pattern duration验证DL-UL模式持续时间
            validDLULPeriodicity{1} =  { 1 2 5 10 }; % Applicable for scs = 15 kHz
            validDLULPeriodicity{2} =  { 0.5 1 2 2.5 5 10 }; % Applicable for scs = 30 kHz  选这个
            ...
            numerology = log2(obj.SCS/15);
            validSet = cell2mat(validDLULPeriodicity{numerology + 1});
            ...
            if isfield(param, 'DLULPeriodicity')
                ...
                if ~ismember(param.DLULPeriodicity, cell2mat(validDLULPeriodicity{numerology + 1}))
                    error('nr5g:hNRScheduler:InvalidDLULPeriodicity','DLULPeriodicity (%.3f) must be one of the set (%s).', ...
                ...
                % Validate the number of full DL slots at the beginning of DL-UL pattern在DL- ul模式的开头验证完整DL插槽的数量
                ...
                % Validate the number of full UL slots at the end of DL-UL pattern在DL- ul模式的开头验证完整UL插槽的数量
        ...
        function updateBufferStatusForGrants(obj, linkType, grants)
            ...
            for grantIdx = 1:length(grants)
                ...
                if linkType % Uplink
                    ...
                    obj.BufferStatusULPerUE(rnti) = obj.BufferStatusULPerUE(rnti) - floor(achievedTxBits/8);
                else % Downlink
                    ...
                    obj.BufferStatusDLPerUE(rnti) = obj.BufferStatusDLPerUE(rnti) - floor(achievedTxBits/8);
                end
            end
        end
    end
