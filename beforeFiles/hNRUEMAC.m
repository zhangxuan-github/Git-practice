...
%#codegen
    ...
    properties
        ...
        %CSIReportPeriodicity CSI reporting (ri-pmi-cqi) periodicity in terms of number of slots
        CSIReportPeriodicity

        %RSRPReportPeriodicity CSI reporting (cri-l1RSRP) periodicity in terms of number of slots
    ...
    properties (Access = private)
        ...
        %SlotsSinceCSIReport Number of slots elapsed since CSI report (ri-pmi-cqi format) was sent
        % It is incremented every slot and as soon as it reaches the
        % 'CSIPeriodicity', it is set to zero and a CQI report is sent
        SlotsSinceCSIReport = 0;

        %SlotsSinceCSIReport Number of slots elapsed since CSI report (cri-rsrp format) was sent
        % It is incremented every slot and as soon as it reaches the
        % 'RSRPReportPeriodicity', it is set to zero and a CSI report is sent
        SlotsSinceRSRPReport = 0;
    ...
    methods
        ...
        function advanceTimer(obj, numSym)
            ...
            if obj.CurrSymbol == 0 % Reached slot boundary
                ...
                if ~isempty(obj.CsirsConfigRSRP)
                    obj.SlotsSinceRSRPReport = obj.SlotsSinceRSRPReport + 1;
                end
            end
        end
        ...
        function dlControlRequest(obj)
            ...
            if(obj.DuplexMode == 0 || ~isempty(find(obj.DLULSlotFormat(obj.CurrDLULSlotIndex + 1, :) == obj.DLType, 1)))
                ...
                obj.DlControlRequestFcn(dlControlType(1:numDLControlPDUs), dlControlPDUs(1:numDLControlPDUs)); % Send DL control request to Phy
            end
        end
        ...
        function controlTx(obj)
            ...
            if isfield(obj.CSIMeasurement, 'CRI')
                if obj.SlotsSinceRSRPReport >= obj.RSRPReportPeriodicity
                    % Construct packet information
