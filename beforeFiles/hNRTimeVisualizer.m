classdef hNRTimeVisualizer < handle
    %hNRTimeVisualizer Scheduler log visualization
    %   The class implements visualization of logs by querying from the
    %   logger (hNRPhyLogger object).
    %
    %   hNRTimeVisualizer methods:
    %
    %   plotPostSimJitter  - Plot post-simulation Jitter visualization
    %
    %   hNRTimeVisualizer Name-Value pairs:
    %
    ...
    properties
        ...
        %PHYLogger phy logger handle object  TIME
        PHYLogger
    ...
    properties(Hidden)
        ...
        %TimeVisualizationFigHandle Handle of the time visualization TIME
        EnableTimeGridVisualization = false;
        TimeVisualizationFigHandle
    ...
    properties (Constant)
        ...
        ExcelFileName = './log_data/table-dlul-crc.xls';
    ...
    methods
        function obj = hNRTimeVisualizer(simParameters,  varargin)
            %hNRTimeVisualizer Construct scheduling log visualization object
            %
            % OBJ = hNRTimeVisualizer(SIMPARAMETERS) Create grid
            % visualization object. It creates figures for visualizing both
            % downlink and uplink information.
            %
            % OBJ = hNRTimeVisualizer(SIMPARAMETERS, ISLOGANALYSIS) Create
            ...
            % Validate number of frames in simulation总仿真帧数
            ...
            for idx = 1: numel(obj.PlotIds)
                ...
                obj.TimeInfo{logIdx} = zeros(obj.NumUEs,obj.MaxPacketCount); %TIME
            end
        ...
        function constructTimeVisualization(obj, varargin)
            %constructtimeVisualization Construct TIME visualization
            %
            % constructtimeVisualization(OBJ, GLOBJ) Construct time visualization
            %

            if nargin == 2
                g = varargin{1};
            else
                g = uigridlayout(obj.TimeVisualizationFigHandle);
                g.ColumnWidth = {100,100,'1x'};
                g.RowHeight = {22,22,22,22,22,22,22,'1x'};
            end
            ...
            if obj.VisualizationFlag == 2
                ...
                % Link direction
                dd1 = uidropdown(g,'Items',{'Downlink','Uplink'}, 'ItemsData', obj.PlotIds, 'ValueChangedFcn', @(dd, event) cbSelectedLinkType(obj, dd.Value, hAx));
                dd1.Layout.Row = 3;
                dd1.Layout.Column = 2;
                compCounter = dd1.Layout.Row;
            else
            ...
            % Number of UEs to be displayed in the default view of time visualization
            
            if ~isempty(obj.NumUEs)
                compCounter = compCounter + 1;
                lb2 = uilabel(g,'Text','Select UE');
                lb2.Layout.Row = compCounter;
                lb2.Layout.Column = 1;
                [items, itemsData] = constructUEItemList(obj);
                dd2 = uidropdown(g,'Items',items , 'ItemsData', itemsData, 'ValueChangedFcn', @(dd, event) rbSelectedUeNum(obj, dd.Value, hAx));
                dd2.Layout.Row = compCounter;
                dd2.Layout.Column = 2;
            end


            % If post simulation log analysis enabled  TIME
            if ~obj.IsAppVisualization
                compCounter = compCounter + 1;
                lb4  = uilabel(g, 'Text','Max Jitter');
                lb4.Layout.Row = compCounter;
                lb4.Layout.Column = 1;
                if ~obj.IsAppVisualization
                    obj.CVTxtHandle = uieditfield(g, 'numeric', 'Value', 0, 'ValueChangedFcn', @(dd, event) showMaxJitter(obj, dd.Value));
                else
                    obj.CVTxtHandle = uilabel(g, 'Text', ' ');
                end
                obj.CVTxtHandle.Layout.Row = compCounter;
            ...
            for idx = 1:min(obj.NumLogs, numel(obj.PlotIds))
                ...
                % Construct the drop down based on number of UEs
                if ~isempty(obj.NumUEs)
                    [obj.UEItemsList(plotId), ~] = constructUEItemList(obj);
                end
            end

            if ~obj.IsAppVisualization
                lb5 = uilabel(g, 'Text', ['Transmission Jitter for Cell ID - '  num2str(obj.CellOfInterest)],  ...
                    'FontSize', 15, 'FontWeight', 'bold', 'WordWrap', 'on');
                lb5.Layout.Row = [2 3];
                lb5.Layout.Column = 5;
            end
            ...
            hAx.Layout.Row = [4 14];
            drawnow;
            obj.TimeJitterTextHandles  = gobjects(obj.NumSlotsFrame, maxRBs);
            xlabel(hAx, 'Packet number');
            ylabel(hAx, 'Jitter -us');
            hAx.XTickLabelRotation = 0;
            hAx.YTickLabelRotation = 0;
            hAx.ZTickLabelRotation = 0;
            drawnow;
        end
    ...
    methods( Access = private)
        function TimeData = LoadTableofTime(obj, filename)
             all_time_data = xlsread(filename);
             [maxrow, column] = 


        end
        function JitterData = GetTimeJitterData(obj, TimeData)
            % obj.JitterData{UEidx} = 
        ...
        function [itemList, itemData] = constructUEItemList(obj)
            %constructUEItemList Create the items for the drop-down component

            % Create the items for the drop-down component
            numUEs = obj.NumUEs;
            itemList = cell(numUEs, 1);
            itemData = zeros(numUEs, 1);
            for i = 1 : numUEs
                itemData(i) = i;
                itemList{i} = ['UE ','-' num2str(itemData(i))];
            end
        end
        ...
        function setupGUI(obj, simParameters)
            ...
            %TIME
            if isfield(simParameters, 'TimeVisualization')
                if islogical(simParameters.TimeVisualization)
                    % To support true/false
                    validateattributes(simParameters.TimeVisualization, {'logical'}, {'nonempty', 'scalar'}, 'simParameters.TimeVisualization', 'TimeVisualization');
                else
                    % To support 0/1
                    validateattributes(simParameters.TimeVisualization, {'numeric'}, {'nonempty', 'integer', 'scalar', '>=', 0, '<=', 1}, 'simParameters.TimeVisualization', 'TimeVisualization');
                end
                obj.EnableTimeGridVisualization = simParameters.TimeVisualization;
            end
            ...
            if(obj.EnableTimeVisualization) % Create  time visualizationTIME
                obj.TimeVisualizationFigHandle = uifigure('Name', 'Transmission Jitter', 'Position', [screenWidth * 0.05 screenHeight * 0.05 figureWidth figureHeight], 'HandleVisibility', 'on');
                constructTimeVisualization(obj);
                if ~isempty(obj.PHYLogger)
                    addDepEvent(obj.PHYLogger, @obj.plotRBGrids, obj.NumSlotsFrame); % Invoke for every frame
                end
            end
