classdef (Abstract) hNRNode < comm_sysmod.internal.wirelessNode
    ...
    properties (Access = public)
        ...
        % Specify this property as a mobility object. This object is
        % invoked to determine current position and velocity of the node,
        % whenever queried.
         Mobility
    ...
    methods
        ...
        function value = position(obj)
            %position Get the current position of node

            if ~isempty(obj.Mobility) % Query current position from mobility model
                value = obj.Mobility.Position(obj.getCurrentTime);
            else
                value = obj.Position;
            end
        end

        function value = velocity(obj)
            %velocity Get the current velocity of node

            if ~isempty(obj.Mobility) % Query current velocity from mobility model
                value = obj.Mobility.Velocity(obj.getCurrentTime);
            else
                value = obj.Velocity;
            end
        end
        ...
        function set.Mobility(obj, mobilityModelObj)
            if isvalid(mobilityModelObj)
                obj.Mobility = mobilityModelObj; % Handle of the mobility object
            else
                error('hNRNode:InvalidObjectHandle', 'Input argument must be a valid object handle');
            end
        end
        ...
        function value = getNodeDistance(obj, refPosition)
            ...
            if ~isempty(obj.DistanceCalculatorFcn)
                value = obj.DistanceCalculatorFcn(refPosition, position(obj));
            else
                % Use Euclidean distance for distance calculation if
                % function handle is not specified
                value = norm(refPosition - position(obj));
            end
        end
    end
