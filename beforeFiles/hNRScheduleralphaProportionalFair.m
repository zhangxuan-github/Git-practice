classdef hNRScheduleralphaProportionalFair < hNRScheduler
    %hNRScheduleralphaProportionalFair Implements proportional fair scheduler

    %   Copyright 2020-2021 The MathWorks, Inc.

    properties
        % MovingAvgDataRateWeight Moving average parameter to calculate the
        % average data rate移动平均参数
    ...
    methods
        function obj = hNRScheduleralphaProportionalFair(simParameters)
            %hNRScheduleralphaProportionalFair Construct an instance of this class
        ...
        function [selectedUE, mcsIndex] = runSchedulingStrategy(obj, schedulerInput)
            ...
            %运行的比例公平算法，并返回UE取得的这个特定的资源块组，以及基于通道条件的合适MCS指数。
            % 这个函数会被调用，为每个RBG选择一个UE用于新的传输，也就是说，在重新传输的分配完成后，为每个剩余的RBG选择一次。
            %根据PF调度策略表示PF权重最大的UE，
            %
            ...
            for i = 1:length(schedulerInput.eligibleUEs)
                ...
                switch(schedulerInput.eligibleUEs(i))
                    case 1
                        alpha = 1;
                    case 2
                        alpha = 2;
...
if(bufferStatus > 0) % Check if UE has any data pending
                    ...
                    %更改位置
                    pfWeightage = achievableDataRate/(pastDataRate^alpha);
    ...
    methods(Access = protected)
        ...
        function uplinkGrants = scheduleULResourcesSlot(obj, slotNum)
            ...
            %一个时隙的Schedule UL资源作为输出返回，传达上行调度器分配资源到不同终端的方式。
            % 'slotNum'是正在被调度的10ms帧中的槽号。
            % 输出'uplnkGrants'是一个单元格数组，其中每个单元格元素代表一个上行授权，有以下字段:
            %
        ...
        function downlinkGrants = scheduleDLResourcesSlot(obj, slotNum)
            ...
            %   context.完成DL时隙调度，还更新了DL HARQ流程上下文。
            %   
    ...
    methods(Access = private)
        function updateUEServedDataRate(obj, linkType, resourceAssignments)
            %updateUEServedDataRate Update UEs' served data rate based on
            %RB assignments更新用户速率
