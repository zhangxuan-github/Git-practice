...
% 指定接收机NVAR的估计噪声方差为非负标量。缺省情况下，如果不指定nVar值作为输入，则认为其值为1e-10。
%
...
%SINRTABLE是以分贝(dB)为单位的15个SINR值的矢量，每个值对应一个CQI值，
% CQI值是根据TS 38.214章节5.2.2.1中提到的块错误率(BLER)条件计算出来的。
% 默认情况下，对应于TS 38.214表5.2.2.1-2的10% BLER条件的
% SINR值是通过对单输入单输出(SISO)场景的加性白高斯噪声(AWGN)信道进行BLER模拟获得的。
%
...
%   % scenario over TDL Channel.这个例子演示了如何通过TDL通道计算4 × 4 mimo场景的CQI。
%
...
%   chInfo = info(channel);
%
%   % Calculate the maximum channel delay计算最大通道延迟
