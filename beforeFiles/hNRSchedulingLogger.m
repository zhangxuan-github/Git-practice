classdef hNRSchedulingLogger < handle    %句柄类
    %hNRSchedulingLogger Scheduler logging mechanism
    ...
    properties
        ...
        %Bandwidth Carrier bandwidth——NumRBs资源块数量
        ...
        %SchedulingLog 仿真的逐个符号记录在FDD模式下，第一个元素包含下行链路调度信息，而第二个元素包含上行链路调度
        %的信息。在TDD模式下，第一个元素包含调度下行链路和上行链路的%
    ...
    properties (Access = private)%只能被自己的类访问
        ...
        %SymbolInfo关于每个符号(UL/DL/Guard)如何分配的信息
        SymbolInfo
        ...
        %StepSize Represents the granularity of logs代表日志的颗粒度
        StepSize
        ...
        % and pending buffer amount
        % bytes.N*3的数组，每列分别表示传输的吞吐量字节，传输的有效吞吐量字节，被积压的数目
        UEMetricsUL
        ...
        % of UEs in each cell and the number of RBs respectively.M和N分别代表每个小区的UE数量和RB数量。
        ...
        % in the uplink。M和N分别代表UE数量和每个用户的HARQ进程数量，每列存着上行的最新的NDI值
        HARQProcessStatusUL
    ...
    methods
        function obj = hNRSchedulingLogger(simParameters, varargin)   %当函数执⾏时,varargin就是⼀个1 * n 的cell数组
            %hNRSchedulingLogger Construct scheduling information logging object
            %
            ...
            % information logging object. If the DuplexMode is set to 1
            % (TDD), set the Flag value to 2.
            %
            % OBJ = hNRSchedulingLogger(SIMPARAMETERS, FLAG, ISLOGREPLAY)
            % Create scheduling information logging object. If the
            % DuplexMode is set to 1 (TDD), set the Flag value to 2.
            %
            ...
            %                     number of symbols (for symbol-based scheduling)最小的TTI颗粒度，按符号数量的百分比（对于基于符号的调度）
            %
            ...
            % frame-by-frame (or slot-by-slot).放模拟的资源分配情况逐帧（或逐槽）。
            ...
            % frame (or slot).以选择一个特定的帧 （或槽），以查看所选帧（或槽）的资源分配方式。
            ...
            else % Validate the number of transmitter antennas on UEs
                invTxAntConfigIdx = find(~ismember(simParameters.UETxAnts, [1,2,4,8,16]));
                if ~isempty(invTxAntConfigIdx)
                    error('nr5g:hNRSchedulingLogger:InvalidAntennaSize',...
                        'Number of UE Tx antennas (%d) must be a member of [1,2,4,8,16].', simParameters.UETxAnts(invTxAntConfigIdx(1)));
                end
            end
            if ~isfield(simParameters, 'UERxAnts')
                simParameters.UERxAnts = ones(simParameters.NumUEs, 1);
            else % Validate the number of receiver antennas on UEs
                invRxAntConfigIdx = find(~ismember(simParameters.UERxAnts, [1,2,4,8,16]));
                if ~isempty(invRxAntConfigIdx)
                    error('nr5g:hNRSchedulingLogger:InvalidAntennaSize',...
                        'Number of UE Rx antennas (%d) must be a member of [1,2,4,8,16].', simParameters.UERxAnts(invRxAntConfigIdx(1)));
                end
            end
            ...
            obj.UplinkChannelQuality = cell(simParameters.NumUEs, 1);
            obj.DownlinkChannelQuality = cell(simParameters.NumUEs, 1);
            ...
            % Initialize number of RBs, RBG size, CQI and metrics
            % properties 决定RBG size的过程
            ...
            for idx=1:min(obj.NumLogs, numel(obj.PlotIds))
                ...
                % Construct the log format  构造日志格式
                obj.SchedulingLog{logIdx} = constructLogFormat(obj, logIdx, simParameters);
            end

            % Check if it is post simulation analysis检查是否为模拟后的分析
            if nargin == 3
                obj.IsLogReplay = varargin{2};
            end

            % Construct the grant log format  构造授权日志格式
            obj.GrantLog = constructGrantLogFormat(obj, simParameters);

            if ~isempty(obj.IsLogReplay) && obj.SchedulingType == obj.SlotBased
                % Post simulation log visualization and slot based scheduling仿真后日志可视化和基于时隙的调度
                obj.StepSize = 1;
                obj.LogInterval = 1;
            else
                % Live visualization
                obj.LogInterval = obj.NumSym;  %14
        ...
        function [dlMetrics, ulMetrics, cellMetrics] = getMACMetrics(obj, firstSlot, lastSlot, rntiList)
            ...
            %
            %   RNTI - Radio network temporary identifier of the UE终端无线网络临时标识符
            %
            %   TxBytes - Total number of bytes transmitted (newTx and reTx combined)发送的总字节数(newTx和reTx的总和)
            %
            %   NewTxBytes - Number of bytes transmitted (only newTx)
            %
            %   BufferStatus - Current buffer status of the UE终端当前缓冲区状态
            %
            %   AssignedRBCount - Number of resource blocks assigned to the UE分配给终端的资源块数
            %
            ...
            %   reTx combined) in downlink 传输的总字节数(newTx和reTx合并)在下行链路
            %
            ...
            for idx = 1:min(obj.NumLogs, numel(obj.PlotIds))
                ...
                % properties读取每个时隙的信息并更新属性
                for i = stepLogStartIdx:obj.StepSize:stepLogEndIdx
                    slotLog = obj.SchedulingLog{schedLogIdx}(i, :);%第几个时隙slot
                    ...
                    % Calculate the RBs allocated to an UE计算每个用户分配的RB块
                    for ueIdx = 1 : obj.NumUEs
                        numRBGs = sum(rbgAssignment(ueIdx, :));%第i个用户的RBG总数
            ...
            % Extract required metrics of the UEs specified in rntiList提取rntiList中指定的ue所需的度量值
            for idx = 1:numel(obj.PlotIds)
            ...
            dlMetrics = outputStruct(:, obj.DownlinkIdx); % Downlink Info下行信息
            ulMetrics = outputStruct(:, obj.UplinkIdx); % Uplink Info上行信息


            % Cell wide metrics 所有用户的各指标和
...
%             cellMetrics.ULNewTxBytes = sum(macNewTxStep(:, obj.UpgetRBGridsInfolinkIdx));
        ...
        function [resourceGrid, resourceGridReTxInfo, resourceGridHarqInfo, varargout] = getRBGridsInfo(obj, frameNumber, slotNumber)
            ...
            %
            % getRBGridsInfo(OBJ, FRAMENUMBER, SLOTNUMBER) Return the resource grid status返回资源网格状态
            %
            ...
            % resource grid allocation status. 在FDD模式下，第一个元素包含下行链路资源网格分配状态和第二个元素包含上行链路资源网格分配状态。
            % In TDD mode first element contains resource grid allocation status for downlink and uplink.
            ...
            % HARQ information. 在FDD模式下，第一个元素包含下行HARQ信息和第二要素包含上行HARQ的信息。
            % In TDD mode first element contains HARQ information for downlink and uplink. Each element is a 2D
            ...
            % status in uplink for FDD mode.
            % 在FDD模式下第一个元素包含在下行链路传输的状态和第二个元素包含上行链路传输状态。
            % In TDD mode first element
            ...
            % Read the resource grid information from logs从日志中读取资源网格信息
            for idx = 1:min(obj.NumLogs, numel(obj.PlotIds))
                plotId = obj.PlotIds(idx);
                if obj.DuplexMode == obj.FDDDuplexMode
                    logIdx = obj.PlotIds(idx);
                ...
                % Reset the resource grid status重置网格单元
                if obj.SchedulingType % Symbol based scheduling
                    numRows = obj.NumSym;
                else % Slot based scheduling
                ...
                slIdx = 0; % Counter to keep track of the number of symbols/slots to be plotted计数器来跟踪要绘制的符号/slot的数量
                for i = frameLogStartIdx+1:obj.StepSize:frameLogEndIdx % For each symbol in the slot or each slot in the frame
                    ...
                    for j = 1 : obj.NumUEs % For each UE
                        ...
                        % information
                        RBGAllocationBitmap = rbgAssignment(j, :);%一个用户是一行，把这个用户这一行取出来，RBGAllocationBitmap是1x14
        ...
        function logCellSchedulingStats(obj, symbolNum, gNB, UEs, varargin)
            %logCellSchedulingStats Log the MAC layer statistics  记录MAC层统计信息
            %
            ...
            % Read pending buffer (in bytes) on gNB, for all the UEs
            obj.UEMetricsDL(:, 3) = getBufferStatus(gNB); 
            ...
            for ueIdx = 1:obj.NumUEs
                ...
                % Read throughput and goodput bytes transmitted for the UE in the current TTI for logging
                [obj.UEMetricsUL(ueIdx, 1), obj.UEMetricsUL(ueIdx, 2)] = getTTIBytes(UEs{ueIdx});
                % Read pending buffer (in bytes) on UE
                obj.UEMetricsUL(ueIdx, 3) = getBufferStatus(UEs{ueIdx}); 
            end

            %这一节的logScheduling会在后面函数里定义
        ...
        function logScheduling(obj, symbolNumSimulation, resourceAssignments, UEMetrics, UECQIs, HarqProcessStatus, type)
            %logScheduling Log the scheduling operations  记录调度操作
            %
            ...
            % UEs and P represents the number of metrics collected.吞吐量
            % 有效吞吐量 积压状态
            %
            ...
            slotDuration = 10/obj.NumSlotsFrame;%每个slot多长时间，10ms/20slot
            ...
            timestamp = obj.CurrFrame * 10 + (obj.CurrSlot * slotDuration) + (obj.CurrSymbol * (slotDuration / 14));%当前时间
            ...
            for j = 1:length(resourceAssignments)
                ...
                % Calculate row number in the logs, for the Tx start symbol
                ...
                if obj.SchedulingType % Symbol based scheduling
                    ...
                    % Fill the logs from the symbol after Tx start, up to the symbol before Tx end
                ...
                obj.GrantLog{obj.GrantCount, grantLogsColumnIndexMap('CQI on RBs')} = mat2str(UECQIs{assignment.RNTI}.CQI);
            end
        ...
        function varargout = getSchedulingLogs(obj)
            ...
            %getSchedulingLogs 获取整个模拟的每个符号的日志
            %按照数值排序（即列索引）获得列的键（即列名）
        ...
        function [dlStats, ulStats] = getPerformanceIndicators(obj)
            ...
            % efficiency values输出数据速率、频谱、效率值
            %
        ...
        function addDepEvent(obj, callbackFcn, numSlots)
            ...
            % event list
            %
            % CALLBACKFCN - Handle of the function to be invoked被调用函数的句柄
            %
            % NUMSLOTS - Periodicity at which function has to be invoked函数被调用的周期
    ...
    methods( Access = private)
        ...
        function logFormat = constructGrantLogFormat(obj, simParam)
            ...
            % simulation, it might assign resources for future slots which
            % are outside of simulation time. Storing those decisions too
            % 在模拟时间内初始化所有符号的调度日志。最后一次时间调度器在模拟中运行时，
            % 它可能会为模拟时间之外的未来槽分配资源。把这些决定也储存起来
